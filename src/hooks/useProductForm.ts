...
export function useProductForm({ initialData, onSuccess, productType: propProductType }: UseProductFormProps) {
  ...
  const getDefaultValues = () => {
    if (productType === 'laptop') {
      return {
        ...
        backlit_keyboard: "",
        face_recognition: "",
      } as LaptopProductData; // Type assertion to ensure compatibility
    }

    // Ensure all required fields for mobile products have default values
    return {
      name: "",
      brand: "",
      model_name: "",
      price: 0,
      currency: "INR",
      publication_status: "draft" as "draft" | "published", // Properly typed
      display_specs: "",
      processor: "",
      ram: "",
      storage: "",
      ...
      os: "",
      color: "",
      // Add optional fields with empty values
      camera_features: "",
      camera_video: "",
      selfie_camera: "",
      selfie_camera_video: "",
      ...
      screen_size: "",
      buy_link: "",
    } as MobileProductData; // Type assertion to ensure compatibility
  };
  ...
  const onSubmit = async (data: MobileProductData | LaptopProductData) => {
    try {
      ...
      if (!finalData.price || isNaN(Number(finalData.price))) {
        throw new Error('Please fill in all required fields: price');
      }
