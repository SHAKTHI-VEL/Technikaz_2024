...
export function useProductForm({ initialData, onSuccess, productType: propProductType }: UseProductFormProps) {
  ...
  const onSubmit = async (data: MobileProductData | LaptopProductData, isDraft: boolean = false) => {
    try {
      setIsLoading(true);
      ...
      // Add a publication_status field to track if it's a draft
      (finalData as any).publication_status = isDraft ? 'draft' : 'published';
